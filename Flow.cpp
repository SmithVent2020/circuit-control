...
void Flow::read() {
  ...
  const signed long Vsupply     = 5;                    // voltage supplied, mv
  const signed long sensorMin   = 1023L * 0.5 / Vsupply;  // Sensor value at 500 mv
  const signed long sensorMax   = 1023L * 4.5 / Vsupply; // Sensor value at 4500 mv
  const signed long sensorRange = sensorMax - sensorMin;
